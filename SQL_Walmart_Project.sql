...
-- Result: January with 116291.8680, Feb: 95727.3765 March: 108867.1500
...
SELECT branch, AVG(total_sales) OVER() AS overall_avg_transaction_amount , 
			   AVG(total_sales) OVER(PARTITION BY branch) AS branch_avg_transaction_amount, 
			   (AVG(total_sales) OVER(PARTITION BY branch))-(AVG(total_sales) OVER()) AS difference
  FROM sales
...
/*
13) Find the top X gross profit margins for the product lines by branch and by month. 
*/
...
SELECT branch, product_line, month_name, SUM(total_sales) OVER(PARTITION BY branch, product_line, month_name) AS product_revenue, 
										 SUM(cogs) OVER(PARTITION BY branch, product_line, month_name) AS product_cogs, 
                                         (SUM(total_sales) OVER(PARTITION BY branch, product_line, month_name)-SUM(cogs) OVER(PARTITION BY branch, product_line, month_name))/(SUM(total_sales) 
															OVER(PARTITION BY branch, product_line, month_name))*100 AS product_line_gpm
		FROM sales
			ORDER BY branch, product_revenue DESC;
 
 -- Result: We simply find that the gpm for the product lines are all the same likely due to cog and total sales being proportional
...
-- 6) What days of the week prove to give the most amount of sales at the different branches?-------------------------------------------------------------------------------------------------------------------
...
SELECT DISTINCT(customer_type), SUM(total_sales) OVER(PARTITION BY customer_type) AS group_expenditure 
  FROM sales
...
*/
SELECT DISTINCT(customer_type), AVG(total_sales) OVER(PARTITION BY customer_type)
  FROM sales;
...
/*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
7)What is the average transaction amount for each payment method? What payment method was used for the smallest transaction? 
What was the largest transaction for this method? What payment method was used for the largest transaction?
*/

SELECT DISTINCT(payment_method), AVG(total_sales) OVER(PARTITION BY payment_method) AS method_avg,
								 MIN(total_sales) OVER(PARTITION BY payment_method) AS method_min,
                                 MAX(total_sales) OVER(PARTITION BY payment_method) AS method_max
  FROM sales;
